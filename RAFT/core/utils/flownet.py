...
def compute_flow_magnitude_tensor(flow):
...
def compute_flow_gradients_tensor(flow):
...
def detect_occlusion_tensor(fw_flow, bw_flow, train_mode=False):
    ...
    ## occlusion
    fb_flow_sum = fw_flow_w + bw_flow
    fb_flow_mag = compute_flow_magnitude_tensor(fb_flow_sum)
    fw_flow_w_mag = compute_flow_magnitude_tensor(fw_flow_w)
    bw_flow_mag = compute_flow_magnitude_tensor(bw_flow)

    mask1 = fb_flow_mag > 0.01 * (fw_flow_w_mag + bw_flow_mag) + 0.5
    
    ## motion boundary
    fx_du, fx_dv, fy_du, fy_dv = compute_flow_gradients_tensor(bw_flow)
    fx_mag = fx_du ** 2 + fx_dv ** 2
    fy_mag = fy_du ** 2 + fy_dv ** 2
    # breakpoint()
    mask2 = (fx_mag + fy_mag) > 0.01 * bw_flow_mag + 0.002
    ...
    occlusion = torch.zeros((fw_flow.shape[0], fw_flow.shape[2], fw_flow.shape[3]), device=fw_flow.device)
    occlusion[mask == 1] = 1
